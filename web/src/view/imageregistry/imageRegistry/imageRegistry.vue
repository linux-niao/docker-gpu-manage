  ...
  <el-select v-model="searchInfo.isOnShelf" clearable placeholder="请选择">
    ...
    <div class="gva-table-box">
        <div class="gva-btn-list">
            ...
            <ExportTemplate  template-id="imageregistry_ImageRegistry" />
            <ExportExcel  template-id="imageregistry_ImageRegistry" filterDeleted/>
            <ImportExcel  template-id="imageregistry_ImageRegistry" @on-success="getTableData" />
        </div>
        <el-table
        ref="multipleTable"
        style="width: 100%"
    ...
    <template #default="scope">{{ formatBoolean(scope.row.isOnShelf) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateImageRegistryFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getImageRegistryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteImageRegistryByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateImageRegistryFunc = async(row) => {
    const res = await findImageRegistry({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteImageRegistryFunc = async (row) => {
